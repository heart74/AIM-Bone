...
class F3Net(nn.Module):
    ...
    def _init_FAD_branch(self, size):
        ...
        # init DCT matrix
        self._DCT_all = torch.tensor(DCT_mat(size), requires_grad=False).float().to(self.device)
        ...
        f_base_low = torch.tensor(base_low, requires_grad=False).to(self.device)
        base_middle = [[0. if i + j > size // 8 or i + j <= size // 16 else 1. for j in range(size)] for i in range(size)]
        f_base_middle = torch.tensor(base_middle, requires_grad=False).to(self.device)
        base_high = [[0. if i + j <= size // 8 else 1. for j in range(size)] for i in range(size)]
        f_base_high = torch.tensor(base_high, requires_grad=False).to(self.device)
        self.F_base_filters = [f_base_low, f_base_middle, f_base_high]
        # self.F_base_filters = [f_base_high]

        # init learnable filter
        f_w_low = nn.Parameter(torch.Tensor(size, size))    #.to(self.device))
        f_w_middle = nn.Parameter(torch.Tensor(size, size)) #.to(self.device))
        f_w_high = nn.Parameter(torch.Tensor(size, size))   #.to(self.device))
        self.F_w_filters = nn.ParameterList([f_w_low, f_w_middle, f_w_high])
    ...
    def _init_LFS_branch(self, window_size, stride, M):
        ...
        # init DCT mat
        self._DCT_patch = torch.tensor(DCT_mat(window_size), requires_grad=False).float().to(self.device)
        ...
        self.L_base_filters = [torch.tensor(b_filter, requires_grad=False).to(self.device) for b_filter in base_filters]
        self.L_base_filters[-1]  = self.L_base_filters[-1] + torch.tensor(mat_r_d, requires_grad=False).to(self.device)

        # init learnable filter
        # self.L_w_filters = [nn.Parameter(torch.Tensor(window_size, window_size).to(self.device)) for i in range(M)]
        self.L_w_filters = nn.ParameterList([nn.Parameter(torch.Tensor(window_size, window_size)) for i in range(M)])
    ...
    def load_pretrained_xception(self, pretrained_path='pretrained/xception-43020ad28.pth'):
        # load model
...
if __name__ == "__main__":
    model = F3Net(num_classes=2, mode='FAD', device=None)
    model.load_pretrained_xception('../model/xception-b5690688.pth')
