...
from .transform import dfdc_test_transforms
import albumentations
import pdb

class AIMDataset(torch.utils.data.Dataset):
    def __init__(self, rootPaths, img_size=224, mode="train", sampleNum=-1, rate=None, transform=None, loader="cv", Use_AIM=False, AIM_ver=1, real_prob=0.5, AIM_prob=0.3, batch_equ=0, cal_res=0, level="c23", Is_align=False):
        ...
        for rootPath in self.rootPaths:
            ...
            self.paths += paths
            self.labels += labels
        self.rate = get_rate(self.labels) if rate is None else rate
    ...
    def generate(self):
        ...
        for path, label in zip(self.paths, self.labels):
            ...
            for faceName in self.sample(os.listdir(folder_path), decayRate):
                ...
                self.frameList.append(imgPath)
                self.labelList.append(label)
                # hjr
                if label == 1:
                    self.realList.append(imgPath)
        print("[Selected Dataset] len(videoList):", len(self.paths))
        print("[Selected Dataset] len(FrameList):", len(self))
        print("[Selected Dataset] len(realList) :", len(self.realList))

    def __getitem__(self, index):
        rand_num = random.random()
        if rand_num < self.real_prob:
            # real
            img_idx = self.fs_idx % len(self.realList)
            self.fs_idx += 1
            frame_path = self.realList[img_idx]
            ...
            # fwa
                ...
                frame_path = self.frameList[index]
                frame = self.loader(frame_path)
                frame = np.uint8(frame)
